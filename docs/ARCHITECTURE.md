...
- **좌표 기준은 창의 좌상단이다.** 패널의 앵커와 pivot이 둘 다 화면 좌상단이고, 저장값은
  ...
  모서리가 오르내려서 바꿨다.
- **경계는 현재 창 크기로 계산한다.** 최대 크기를 항상 예약하면 작은 창도 오른쪽·아래쪽에
  큰 빈 영역을 남겨, 2560×1440 화면에서 1920×1080 부근을 넘지 못하는 것처럼 보인다.
  `Resize`가 크기를 바꾼 뒤 위치를 다시 보정하므로 창이 커져도 화면 밖으로 나가지 않는다.
  커진 창 때문에 표시 위치가 안쪽으로 밀려도 원하는 위치(`_desired`)는 유지하며, 다시
  작아지면 사용자가 둔 자리로 돌아간다.
