...
- 창 왼쪽 위 **점 여섯 개 손잡이**를 누른 채 끌면 창이 따라 움직인다. 놓은 자리는
  설정 파일에 저장돼 다음 실행에도 유지된다. 현재 창 크기를 기준으로 화면 전체를 쓸 수
  있고, 로그가 늘어 창이 커질 때는 화면 밖으로 나가지 않게 안쪽으로 보정된다
...
- 실패한 판 시작 응답은 무시하고, 시작 신호를 놓쳐도 방이 바뀌면 한 번 초기화한다
- BepInEx의 `LogOutput.log`에 `[game]` 진단을 남긴다. 판 시작 신호, 방 채택·변경,
  명단 변경 수와 라운드만 기록하며 계정 uid·방 id·닉네임·메시지 본문은 기록하지 않는다
- 2560×1440 같은 고해상도에서 작은 로그창이 화면 오른쪽·아래쪽까지 이동하지 못하던
  문제를 고쳤다
- `Round` 머리줄과 전투 로그 사이의 불필요한 빈 공간을 줄였다

게임을 끄지 않고 매칭 두 판을 이어서 진행한 뒤 `LogOutput.log`를 확인하면 된다. 두 번째
판 앞에 `[game] new game (MatchSuccess)`가 한 번 나오고 참가자 이름·라운드·몬스터 번호가
새로 시작해야 한다. `room changed without a start signal`이 나오면 방 변경 안전망이 작동한
것이다. 이 안전망이 이미 전투 화면에 들어온 뒤 작동하면 오버레이는 다음 라운드에서 뜰 수 있다.
